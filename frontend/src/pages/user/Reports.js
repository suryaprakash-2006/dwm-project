...
export default function Reports() {
  ...
  // Active category filters — stored so active filter display updates correctly
  const [activeCategoryId,    setActiveCategoryId]    = useState(""); // eslint-disable-line no-unused-vars
  const [activeSubCategoryId, setActiveSubCategoryId] = useState(""); // eslint-disable-line no-unused-vars
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (params.categoryId)
          result = result.filter(r => Number(r.workCategoryId) === Number(params.categoryId));
        if (params.subCategoryId)
          result = result.filter(r => Number(r.subCategoryId) === Number(params.subCategoryId));
        setFiltered(result);
      }
    } catch (err) {
  ...
  // -------- Reset --------
  const handleReset = () => {
    setTempShift("all"); setTempDateFrom(""); setTempDateTo(""); setTempCategoryId(""); setTempSubCategoryId("");
    setDateError(""); setActiveFilters({}); setActiveCategoryId(""); setActiveSubCategoryId("");
    fetchEntries();
  };
